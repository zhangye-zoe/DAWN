...
class ResUNet(nn.Module):
    def __init__(self, out_c, n_layers=34, pretrained=True, fixed_feature=False):
        ...
        self.fc1 = nn.Linear(7225, 512)
        self.fc2 = nn.Linear(512, 128)
        self.fc3 = nn.Linear(128, 32)
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.relu = nn.ReLU()
        self.sf_conv1 = nn.Conv2d(1,1, 3, stride=3, bias=True)
        # self.sf_conv2 = nn.Conv2d(1,1, 3, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        # print("out",  x.shape)
        # print("="*80)
        # adding cellular number calculation
        # n_x = x.clone()
        # n_x = self.max_pool(x)
        n_x = self.sf_conv1(x)
        dim = n_x.shape[0]
