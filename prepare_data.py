...
def main(opt):
    ...
    # file = '{:s}/train_val_test.json'.format(data_dir)
    ...
    if opt.round == 0:
        ...
        create_detect_label_from_points(label_point_dir, label_detect_dir, train_list, dataset, radius=opt.r1)
    ...
    if opt.round == 0:
        ...
        split_patches(init_mask_dir, '{:s}/init_mask'.format(patch_folder), 'init_mask')
        split_patches(true_mask_dir, '{:s}/true_mask'.format(patch_folder), 'true_mask')
...
def create_detect_label_from_points(label_point_dir, label_detect_dir, img_list, dataset, radius):
    ...
    for image_name in sorted(img_list):
        ...
        if np.sum(points > 0):
            ...
            det_num_point_dict[name] = np.sum(points>0)
            seg_num_point_dict[name] = np.sum(seg_points>0)
        else:
            label_detect = np.zeros(points.shape)
            det_num_point_dict[name] = 0
...
def organize_data_for_training(data_dir, train_data_dir, opt):
    ...
    create_folder('{:s}/init_mask/train'.format(train_data_dir))
    create_folder('{:s}/true_mask/train'.format(train_data_dir))
    ...
    if opt.round == 0:
        ...
        for img_name in train_list:
            ...
            for file in glob.glob('{:s}/patches/labels_detect/{:s}_*'.format(data_dir, name)):
                ...
                shutil.copyfile(file, dst)
            # init mask 
            for file in glob.glob('{:s}/patches/init_mask/{:s}_*'.format(data_dir, name)):
                file_name = file.split('/')[-1]
                dst = '{:s}/init_mask/train/{:s}'.format(train_data_dir, file_name)
                shutil.copyfile(file, dst)

            for file in glob.glob('{:s}/patches/true_mask/{:s}_*'.format(data_dir, name)):
                file_name = file.split('/')[-1]
                dst = '{:s}/true_mask/train/{:s}'.format(train_data_dir, file_name)
                shutil.copyfile(file, dst)
