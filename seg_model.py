...
class HoVerNet(Net):
    ...
    def __init__(self, input_ch=3, nr_types=None, freeze=False, mode='fast'):
        super().__init__()
        self.mode = mode
        self.freeze = freeze
        ...
        self.conv1 =  nn.Conv2d(4, 4, 3, stride=3, bias=True)
        # self.conv2  = nn.Conv2d(2, 2, 3, stride=1, bias=True)
    ...
    def forward(self, imgs):
        ...
        for branch_name, branch_desc in self.decoder.items():
            ...
            if branch_name == "np":
                # np_u0 =  self.maxpool(u0)
                np_u0 = u0
                # print("np_u0", np_u0.shape)

                # dim = np_u0.shape[0]
                # np_u0 = np_u0.view(dim, -1)

            if branch_name == "hv":
                hv_u0 = u0
                # hv_u0 =  self.maxpool(u0)
                # dim = hv_u0.shape[0]
                # hv_u0 = hv_u0.view(dim, -1)
                # print(hv_u0.shape)
        u = torch.concat((np_u0, hv_u0),dim=1)
